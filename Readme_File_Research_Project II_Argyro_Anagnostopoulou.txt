...
Input Files
enrichment_matrix_c2.tsv: Tab-separated file containing Reactome pathway enrichment results from GSEA.

Output Files
filt_overlap_gs_Re_df.tsv: Tab-separated file containing the filtered and ranked TCs associated with immune processes. 
...
Input Files:
enrichment_matrix_c5.tsv: Tab-separated values file containing enrichment matrix from the GOBP database.

Output Files:
filt_overlap_gs_df.tsv: Tab-separated values file containing the selected TCs associated with immune-related gene sets, ranked 1.
...
* GO-BP gene sets with Bonferroni corrected p-values (dataframe).
...
* Subset of GO-BP and Reactome gene sets corresponding to immune TCs (tab-separated files).
* Heatmaps for immune Reactome and GO-BP data (PDF files).
